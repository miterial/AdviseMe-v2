...
import com.lanagj.adviseme.recommender.nlp.lsa.weight.bag_of_words.BagOfWordsDataProvider;
...
class TfIdfTest {
    ...
    void calculateWeight(Map<String, List<BagOfWords.WordFrequency>> wordFrequency) {
        ...
        List<DocumentStats> actual = this.service.calculateWeight(wordFrequency);

        assertEquals(5 * 3, actual.size()); // 5 words * 3 documents = matrix of size 5x3

        // check tf-idf for each document
        List<DocumentStats> actual_1 = actual.stream().filter(wd -> wd.getWord().equals("word1")).collect(Collectors.toList());
        ...
        List<DocumentStats> actual_2 = actual.stream().filter(wd -> wd.getWord().equals("word2")).collect(Collectors.toList());
        ...
        List<DocumentStats> actual_3 = actual.stream().filter(wd -> wd.getWord().equals("word3")).collect(Collectors.toList());
        ...
        List<DocumentStats> actual_4 = actual.stream().filter(wd -> wd.getWord().equals("word4")).collect(Collectors.toList());
        ...
        List<DocumentStats> actual_5 = actual.stream().filter(wd -> wd.getWord().equals("word5")).collect(Collectors.toList());
